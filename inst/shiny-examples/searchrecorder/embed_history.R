...
#' @param save Logical argument specifying whether to save the RIS 
#' file to the working directory. Default is set to TRUE.
#' @importFrom utils write.table
#' @return An RIS file containing the search record embedded in the 
#' custom field of the first record in the file.
#' @examples 
#' \dontrun{
#' embed_history(ris_input = 'inst/extdata/references.ris', search_record = 'inst/extdata/search_record.JSON')
#' }
embed_history <- function(ris_input,
  ...
  ris_output <- c(start, 
                  paste0('C1  - search_history_start', search_history, 'search_history_end'), 
                  rest)
  ...
  if(save == TRUE) {
    ...
    write(ris_output, file = "ris_output.ris")
